...
export default function ImprovementSuggestionsCard({ suggestions = [] }) {
  ...
  const getSeverityBadge = (severity) => {
    ...
    if (sev === 'low' || sev === 'minor' || sev === 'praise') {
      return (
  ...
  const getCategoryIcon = (category) => {
    ...
    if (cat.includes('clear') || cat.includes('praise')) {
    ...
    <div className="bg-white dark:bg-[#0f172a] rounded-3xl border border-slate-200/90 dark:border-slate-800 p-6 sm:p-8 shadow-card space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 pt-1">
        {suggestions.map((item, index) => {
          const text = item.description || item.suggestion || '';
          ...
          return (
            ...
            >
              ...
              <div className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed pl-6 [&>p]:inline [&>p]:leading-relaxed">
